...
func (_c *PostCreate) SetBoardID(id int) *PostCreate {
	_c.mutation.SetBoardID(id)
	return _c
}

// SetBoard sets the "board" edge to the DiscussionBoard entity.
func (_c *PostCreate) SetBoard(v *DiscussionBoard) *PostCreate {
	return _c.SetBoardID(v.ID)
}

// AddReplyIDs adds the "replies" edge to the Reply entity by IDs.
func (_c *PostCreate) AddReplyIDs(ids ...int) *PostCreate {
	_c.mutation.AddReplyIDs(ids...)
	return _c
}

// AddReplies adds the "replies" edges to the Reply entity.
func (_c *PostCreate) AddReplies(v ...*Reply) *PostCreate {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
...
func (_c *PostCreate) check() error {
	...
	if len(_c.mutation.BoardIDs()) == 0 {
		return &ValidationError{Name: "board", err: errors.New(`ent: missing required edge "Post.board"`)}
	}
	return nil
}
...
func (_c *PostCreate) createSpec() (*Post, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.BoardIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
		...
		_node.discussion_board_posts = &nodes[0]
		_spec.Edges = append(_spec.Edges, edge)
	}
