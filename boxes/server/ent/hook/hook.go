...
// The DiscussionBoardFunc type is an adapter to allow the use of ordinary
// function as DiscussionBoard mutator.
type DiscussionBoardFunc func(context.Context, *ent.DiscussionBoardMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f DiscussionBoardFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.DiscussionBoardMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.DiscussionBoardMutation", m)
}
