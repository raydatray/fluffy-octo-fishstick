...
func (_q *ReplyQuery) QueryAuthor() *UserQuery {
...
func (_q *ReplyQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *ReplyQuery) FirstIDX(ctx context.Context) int {
...
func (_q *ReplyQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *ReplyQuery) OnlyIDX(ctx context.Context) int {
...
func (_q *ReplyQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (_q *ReplyQuery) IDsX(ctx context.Context) []int {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *ReplyQuery) WithAuthor(opts ...func(*UserQuery)) *ReplyQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *ReplyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Reply, error) {
	...
	if _q.withAuthor != nil || _q.withPost != nil {
		withFKs = true
	}
	if withFKs {
...
func (_q *ReplyQuery) loadAuthor(ctx context.Context, query *UserQuery, nodes []*Reply, init func(*Reply), assign func(*Reply, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Reply)
	for i := range nodes {
		if nodes[i].user_replies == nil {
			continue
		}
		fk := *nodes[i].user_replies
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_replies" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ReplyQuery) loadPost(ctx context.Context, query *PostQuery, nodes []*Reply, init func(*Reply), assign func(*Reply, *Post)) error {
	...
	nodeids := make(map[int][]*Reply)
	for i := range nodes {
		if nodes[i].post_replies == nil {
			continue
		}
		fk := *nodes[i].post_replies
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "post_replies" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
