...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	...
	case coursesection.Table:
		query := c.CourseSection.Query().
			Where(coursesection.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, coursesectionImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
	case discussionboard.Table:
		query := c.DiscussionBoard.Query().
			Where(discussionboard.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, discussionboardImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
	...
	case reply.Table:
		query := c.Reply.Query().
			Where(reply.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, replyImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
...
func (c *Client) noders(ctx context.Context, table string, ids []int) ([]Noder, error) {
	...
	case coursesection.Table:
		query := c.CourseSection.Query().
			Where(coursesection.IDIn(ids...))
		query, err := query.CollectFields(ctx, coursesectionImplementors...)
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
	...
	case discussionboard.Table:
		query := c.DiscussionBoard.Query().
			Where(discussionboard.IDIn(ids...))
		query, err := query.CollectFields(ctx, discussionboardImplementors...)
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
	...
	case reply.Table:
		...
		query, err := query.CollectFields(ctx, replyImplementors...)
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
