...
func (_q *PostQuery) QueryBoard() *DiscussionBoardQuery {
...
func (_q *PostQuery) QueryReplies() *ReplyQuery {
...
func (_q *PostQuery) WithBoard(opts ...func(*DiscussionBoardQuery)) *PostQuery {
	query := (&DiscussionBoardClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PostQuery) WithReplies(opts ...func(*ReplyQuery)) *PostQuery {
	query := (&ReplyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if _q.withAuthor != nil || _q.withBoard != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withBoard; query != nil {
		if err := _q.loadBoard(ctx, query, nodes, nil,
			func(n *Post, e *DiscussionBoard) { n.Edges.Board = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *PostQuery) loadBoard(ctx context.Context, query *DiscussionBoardQuery, nodes []*Post, init func(*Post), assign func(*Post, *DiscussionBoard)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Post)
	for i := range nodes {
		if nodes[i].discussion_board_posts == nil {
			continue
		}
		fk := *nodes[i].discussion_board_posts
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(discussionboard.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "discussion_board_posts" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *PostQuery) loadReplies(ctx context.Context, query *ReplyQuery, nodes []*Post, init func(*Post), assign func(*Post, *Reply)) error {
	...
	for _, n := range neighbors {
		fk := n.post_replies
		if fk == nil {
			return fmt.Errorf(`foreign-key "post_replies" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "post_replies" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
